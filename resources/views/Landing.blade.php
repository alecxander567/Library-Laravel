...
<body class="flex flex-col min-h-screen" x-data="{ loginOpen: false, signupOpen: false } ">
    ...
    <header class="bg-blue-700 text-white py-4 shadow-md">
        <div class="container mx-auto flex justify-between items-center px-6">
            ...
            <div class="flex space-x-4">
                <button @click="loginOpen = true" class="bg-white text-blue-700 px-4 py-2 rounded hover:bg-gray-100 transition font-semibold">
                    Login
                </button>
                <button @click="signupOpen = true" class="bg-green-500 text-white px-4 py-2 rounded hover:bg-green-600 transition font-semibold">
                    Sign Up
                </button>
            </div>
        </div>
    ...
    <section class="hero-bg bg-gradient-to-r from-blue-600 via-blue-500 to-blue-400 py-32">
        <div class="flex flex-col md:flex-row items-center justify-between max-w-6xl mx-auto px-6 md:gap-16 gap-10">
            ...
            <a href="/books" class="bg-blue-700 text-white px-10 py-5 rounded-xl font-bold text-lg hover:bg-blue-800 transition">
                Browse Books
            </a>
            </div>

            <div class="max-w-md md:max-w-lg">
            <img src="https://images.unsplash.com/photo-1512820790803-83ca734da794?auto=format&fit=crop&w=800&q=80" alt="Library" class="rounded-xl shadow-lg w-full object-cover">
            </div>
        </div>
    </section>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                <div class="bg-white p-6 rounded-lg shadow hover:shadow-lg transition flex flex-col items-center">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-12 w-12 text-blue-600 mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16h8M8 12h8m-6-8h6a2 2 0 012 2v16H6V6a2 2 0 012-2h6z" />
                    </svg>
                ...
                <div class="bg-white p-6 rounded-lg shadow hover:shadow-lg transition flex flex-col items-center">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-12 w-12 text-green-600 mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-6a2 2 0 012-2h2a2 2 0 012 2v6M5 9h14M5 13h14M5 17h14" />
                    </svg>
    ...
    <div x-show="loginOpen" x-transition class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
        <div @click.away="loginOpen = false" class="bg-white p-8 rounded-lg shadow-lg w-96">
            ...
            <form action="{{ route('login.perform') }}" method="POST" class="flex flex-col space-y-4">
                @csrf
                <input type="email" name="email" placeholder="Email" value="{{ old('email') }}" class="border p-2 rounded w-full">
                <input type="password" name="password" placeholder="Password" class="border p-2 rounded w-full">

                <button type="submit" class="bg-blue-700 text-white px-4 py-2 rounded hover:bg-blue-800 transition font-semibold">
                    Login
                </button>
            </form>
    ...
    <div x-show="signupOpen" x-transition class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
        <div @click.away="signupOpen = false" class="bg-white p-8 rounded-lg shadow-lg w-96">
            ...
            <form action="{{ route('register.store') }}" method="POST" class="flex flex-col space-y-4">
                @csrf
                <input type="text" name="name" placeholder="Full Name" value="{{ old('name') }}" class="border p-2 rounded w-full">
                <input type="email" name="email" placeholder="Email" value="{{ old('email') }}" class="border p-2 rounded w-full">
                <input type="password" name="password" placeholder="Password" class="border p-2 rounded w-full">
                <input type="password" name="password_confirmation" placeholder="Confirm Password" class="border p-2 rounded w-full">

                <button type="submit" class="bg-green-500 text-white px-4 py-2 rounded hover:bg-green-600 transition font-semibold">
                    Sign Up
                </button>
            </form>
